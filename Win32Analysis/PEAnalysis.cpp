...
PEAnalysis::PEAnalysis(TCHAR * pSzName)
{
	m_szExePath = pSzName;
	if (!AnalysisPe())
	{
...
PIMAGE_DOS_HEADER PEAnalysis::GetDosHeader()
{
	return (PIMAGE_DOS_HEADER)m_pbuff;
}

PIMAGE_NT_HEADERS PEAnalysis::GetNtHeader()
{
	// nt = dos.e_lfnew + pbuff;
	return  (PIMAGE_NT_HEADERS)(GetDosHeader()->e_lfanew + (LONG)m_pbuff);
}

PIMAGE_SECTION_HEADER PEAnalysis::GetSectionHeader()
{
	return IMAGE_FIRST_SECTION(GetNtHeader());
}

DWORD PEAnalysis::RvaToOfset(DWORD RVA)
{
...
PIMAGE_EXPORT_DIRECTORY PEAnalysis::GetExportDirectory()
{
	//数据目录表第0项
...
PIMAGE_IMPORT_DESCRIPTOR PEAnalysis::GetImportDescriptor()
{
	//数据目录表第0项
...
BOOL PEAnalysis::GetExprotInfo(vector<ExportInfo>& vec_info)
{
	//解析PE导表 
...
BOOL PEAnalysis::GetIATInfo(vector<IATInfo>& vec_info)
{
...
PIMAGE_RESOURCE_DIRECTORY PEAnalysis::GetResourceDirectory()
{
...
BOOL PEAnalysis::GetResourceInfo(vector<ResourceInfo>& vec_ResInfo)
{
...
PIMAGE_BASE_RELOCATION PEAnalysis::GetRelocation()
{
...
BOOL PEAnalysis::GetRelocationInfo(vector<RelocationInfo>& vec_RelInfo)
{
